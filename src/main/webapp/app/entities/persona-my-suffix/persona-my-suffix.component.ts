...
@Component({
    selector: 'jhi-persona-my-suffix',
    templateUrl: './persona-my-suffix.component.html'
})
export class PersonaMySuffixComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    loadAll() {
        this.personaService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<PersonaMySuffix[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/persona-my-suffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/persona-my-suffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: PersonaMySuffix) {
        return item.id;
    }
